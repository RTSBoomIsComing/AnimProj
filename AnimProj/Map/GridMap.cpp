...
namespace pa
{
	...
	Actor* GridMap::findNearestActor(World& world, Actor& actor, float radius, std::function<bool(const Actor*)>& predicate) const
	{
		using namespace DirectX;
		...
		for (size_t x = startX; x <= endX; ++x)
		{
			for (size_t z = startZ; z <= endZ; ++z)
			{
				for (Actor* other : _cells[x][z])
				{
					...
					if (distance < minDistance && distance <= radius && predicate(other))
					{
						minDistance	 = distance;
						nearestActor = other;
					}
